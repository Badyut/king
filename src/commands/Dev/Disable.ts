...
export default class Command extends BaseCommand {
    ...
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        if (!command) return void (await M.reply(`No such 🙂 fucking 😈 command found`))
        if (await this.client.DB.disabledcommands.findOne({ command: command.config.command }))
            return void M.reply(`${command.config.command} is already 😒 fool disabled`)
        await new this.client.DB.disabledcommands({
            command: command.config.command,
            reason: (split[1] || '').trim() || ''
        }).save()
        await M.reply(
            `*${this.client.util.capitalize(command.config.command)}* is now 😈 Disabled 🙂 motherfucker${
                split[1] ? ` for ${split[1]}` : ''
            }`
        )
    }
